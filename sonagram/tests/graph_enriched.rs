...
fn track_gains_lastfm_and_original_album_props() {
    ...
    assert_eq!(
        prop(&g, "Track", ABBA_HASH, "lastfm_playcount"),
        Value::Int64(1_800_000)
    );
    assert_eq!(
        prop(&g, "Track", ABBA_HASH, "lastfm_listeners"),
        Value::Int64(320_000)
    );
    assert_eq!(
        prop(&g, "Track", ABBA_HASH, "has_lastfm_match"),
        Value::Boolean(true)
    );
    ...
    assert_eq!(
        prop(&g, "Track", ABBA_HASH, "original_album_position"),
        Value::Int64(7)
    );
...
fn non_enriched_track_has_no_lastfm_props() {
    ...
    assert_eq!(
        prop(&g, "Track", &hash, "has_lastfm_match"),
        Value::Boolean(false)
    );
...
fn popularity_is_stable_under_input_reordering() {
    ...
    let other =
        graph::build_graph_with_enrichment(&reversed, Some(&enrichment), &library()).unwrap();
...
fn artist_and_album_gain_lastfm_props() {
    ...
    assert_eq!(
        prop(&g, "Artist", "ABBA", "lastfm_playcount"),
        Value::Int64(155_000_000)
    );
    assert_eq!(
        prop(&g, "Artist", "ABBA", "lastfm_listeners"),
        Value::Int64(3_200_000)
    );
    assert_eq!(
    ...
    assert_eq!(
        prop(&g, "Album", alb, "lastfm_playcount"),
        Value::Int64(2_600_000)
    );
...
fn folksonomy_extends_genre_dimension_and_in_genre_edges() {
    ...
    assert_eq!(
        counts.get("IN_GENRE").copied().unwrap_or(0),
        23,
        "IN_GENRE total"
    );
}
...
fn crowd_similar_edges_weighted_and_attributed_and_dropped() {
    ...
    let counts = g.get_edge_type_counts();
    assert_eq!(
        counts.get("CROWD_SIMILAR").copied().unwrap_or(0),
        8,
        "CROWD_SIMILAR total"
    );
    ...
    assert_eq!(
        tt, 4,
        "Track→Track CROWD_SIMILAR (non-owned similars dropped)"
    );
    assert_eq!(
        tt_score, 4,
        "every Track→Track CROWD_SIMILAR carries a score weight"
    );
    ...
    assert_eq!(
        aa, 4,
        "Artist→Artist CROWD_SIMILAR (non-owned similars dropped)"
    );
    assert_eq!(
        aa_src, 4,
        "every Artist→Artist CROWD_SIMILAR carries source=lastfm"
    );
    ...
    for e in sg.edge_indices() {
        ...
        if s == Value::String(ABBA_HASH.to_string()) && t == Value::String(BRUNO_HASH.to_string()) {
            match edge.properties_cloned(&g.interner).get("score") {
                Some(Value::Float64(w)) => {
                    assert!(
                        (w - 0.8).abs() < 1e-6,
                        "score weight preserved (0.80), got {w}"
                    );
                    found = true;
                }
...
fn plain_build_has_no_enrichment_values_or_edges() {
    ...
    assert_eq!(
        node_count(&plain, "Genre"),
        10,
        "plain Genre count unchanged"
    );
    let counts = plain.get_edge_type_counts();
    assert_eq!(
        counts.get("IN_GENRE").copied().unwrap_or(0),
        14,
        "plain IN_GENRE unchanged"
    );
    assert_eq!(
        counts.get("CROWD_SIMILAR").copied().unwrap_or(0),
        0,
        "no CROWD_SIMILAR in plain"
    );
    // Counts/rank stay null, while the recognition flag is always false.
    assert_eq!(
        prop(&plain, "Track", ABBA_HASH, "lastfm_playcount"),
        Value::Null
    );
    assert_eq!(
        prop(&plain, "Track", ABBA_HASH, "lastfm_listeners"),
        Value::Null
    );
    assert_eq!(
        prop(&plain, "Track", ABBA_HASH, "has_lastfm_match"),
        Value::Boolean(false)
    );
    assert_eq!(prop(&plain, "Track", ABBA_HASH, "popularity"), Value::Null);
    assert_eq!(
        prop(&plain, "Track", ABBA_HASH, "original_album"),
        Value::Null
    );
}
