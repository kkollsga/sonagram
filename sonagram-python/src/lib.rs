    ...
    let kglite = py.import("kglite").map_err(|e| {
        PyRuntimeError::new_err(format!(
            "sonagram.build()/scan_and_build() return a kglite.KnowledgeGraph, but \
             importing `kglite` failed ({e}). Install it: `pip install kglite>=0.16.12`."
        ))
    })?;
    kglite
...
fn kglite_load_failure_message(installed_version: Option<&str>, original_error: &str) -> String {
    ...
    format!(
        "sonagram built the graph, but handing it to `kglite` failed: {original_error}. \
         sonagram writes `.kgl` files that need kglite>=0.16.12, and {installed} — \
         an older wheel cannot read that container format. \
         Upgrade it: `pip install -U 'kglite>=0.16.12'`."
    )
}
...
mod tests {
    ...
    fn kglite_load_failure_names_the_wheel_version_and_the_upgrade() {
        ...
        assert!(
            msg.contains("pip install -U 'kglite>=0.16.12'"),
    ...
    fn kglite_load_failure_survives_a_wheel_with_no_version_attribute() {
        ...
        assert!(msg.contains("pip install -U 'kglite>=0.16.12'"), "{msg}");
    }
