...
| 2026-08-20 | `721f750c1231`    | KGLite 0.16.3 → 0.16.5 move (two upstream releases in one bump) — **no digest change** (graph schema remains v3). Both goldens byte-identical (plain `721f750c1231`, enriched `c3aa41d6a910`); `capture_goldens` never run; the goldens are untouched in the working tree. The v6 container does not move for us because every format-affecting addition is opt-in and we opt into none: property-type constraints (0.16.4) and relationship constraints (0.16.5) are enforced only where declared and we declare none, so our `.kgl` files stay loadable by older kglite rather than tripping 0.16.4's format note, and a graph that never enables CDC writes no epoch stamp. The one change reaching our source is 0.16.5's BREAKING Rust API move of node/relationship/map properties from `BTreeMap<String, Value>` to `PropMap`; it is source-compatible here because our only property access is `node.properties.get(...)` (`sonagram/src/playlist.rs:482,498`), which `PropMap` serves, and the type preserves key-ordered iteration, `Eq`, `Ord` and hashing — so the determinism guarantee the digest rests on is unchanged. `contract_kglite` green against the real API. Perf: the only cell that moved is `ORDER BY` over 5k materialised nodes at -11.0%/-11.9% wall across two agreeing runs (PropMap's refcount-instead-of-deep-copy win); scan+filter+project, graph load + curation profile, and the kglite-free control are all flat, and every cell's output is byte-identical PRE→POST. |
| 2026-08-17 | `721f750c1231`    | KGLite 0.16.2 → 0.16.3 + Sonara 0.3.5 → 0.3.6 moves — **no digest change** (graph schema remains v3). KGLite 0.16.3 is MCP-server-layer only (tool surface, reload/watch; engine and v6 container untouched); Sonara 0.3.6 adds the per-feature augment API and versioned similarity profiles with the default profile bit-identical to 0.3.5 and no analysis-schema bump. Both goldens byte-identical (plain `721f750c1231`, enriched `c3aa41d6a910`); `capture_goldens` never run. Contract tripwires added for the augment surface (fn-shape pins + profile version probes, all seen red by mutation). |
| 2026-08-16 | `721f750c1231`    | KGLite 0.15.8 → 0.16.2 engine move — **no digest change** (graph schema remains v3). Both goldens byte-identical across the engine's columnar-everywhere storage rewrite and sonagram's NodeView read migration (plain `721f750c1231`, enriched `c3aa41d6a910`); the gate's own node sweep moved to `node_view` in the same change, with the unchanged digest as the pass condition — `capture_goldens` never ran. Cross-version parity additionally verified on a real 32,891-track v5 graph: `profile --format json` SHA-256 byte-identical between the 0.15.8 and 0.16.2 binaries. Note: graphs *saved* by 0.16.2 use `.kgl` container v6, which pre-0.16 kglite tools refuse (one-way; 0.16 still reads v5). |
