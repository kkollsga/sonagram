...
- **`contract_sonara` / `contract_kglite`** — compile-time + runtime assertions
  ...
  genre/vocalness model IDs), `ANALYSIS_SCHEMA_VERSION == 4`,
...
| 2026-07-27 | `079a046fd7ea`    | Sonara 0.3.4 sync — **fingerprint-only** (intended; analysis schema 4→6, similarity v2 / graph schema v2 unchanged). The 15 frozen fixtures were **NOT** re-captured, and that is deliberate: both sonara bumps are aggression-only (v5 "stored aggression values require an audio rescan"; v6 "bundled aggression values are evaluated at the model's canonical 22.05 kHz sample rate. Main-pass fields and frame provenance stay in the caller-requested sample-rate domain"). Sonagram does not enable sonara's opt-in `aggression` feature — every aggression field in `analyze.rs` is `#[cfg(feature = "aggression")]`, and the new `mood.rs` is a documented bit-for-bit move of heuristic v1 out of `perceptual.rs` — so a default-feature sonara 0.3.4 reproduces the frozen main-pass values exactly. The **entire** canonical delta is 2 lines per golden: `Source.build_input_fingerprint` and `Library.build_input_fingerprint`, which mix `ANALYSIS_SCHEMA_VERSION` in by design. No node, edge, embedding, property, or analysis value changed. Fixtures still honestly record `schema_version = 4` (the version they were captured under); rewriting that without audio would fabricate provenance. Plain → `079a046fd7ea`; enriched → `edd5fbe2b168`. |
